...
* 2. 일관성 유지 => 하나의 인스턴스를 공유하기 때문에 데이털의 일관성을 유지
* 3. 전역 상태 관리 => 객체를 전역적으로 접근 가능
* 4. 자원 관리 => 데이터베이스 연결 객체 서버에서 단 하나만 존제해야 하기 때문에 이때 싱글톤을 사용
*
* 단점
* 1. 테스트의 어려움이 있음 => 전역적인 접근성을 가져서 객체간의 의존성이 높아짐, 객체간의 테스트가 어려워 짐
